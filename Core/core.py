...
def find_best_route(source, destination) -> dict:
        ...
        if i + 1 < len(route):
            if check_line(route[i], route[i + 1]) != corrent_line:
                ...
                try :
                    ...
                    print(f"line : {corrent_line} , ter : {terminal_direction} , station : {route[i]} , dest : {terminal_direction} , line : {corrent_line} .e : {e} ")
                if flag and i == 0 :   
                    add_overview_entry(overview, route[i], now, corrent_line, False, "")
                elif flag and i != 0 :
    ...
    return {"route": overview ,"travel_time" : f"{time[0]}:{time[1]}" , "travel_cost" : check_cost(travel_cost), "travel_guide" : travel_guide }
